...
import json, subprocess, time, os, glob, urllib.request, sys
...
def kc_read():
    out = subprocess.run(["security", "find-generic-password", "-s", KC_SERVICE, "-w"],
...
def kc_write(full):
    ...
    subprocess.run(["security", "add-generic-password", "-U", "-s", KC_SERVICE,
...
def claude_feed():
    ...
    try:
        with open(FEED) as f:
            d = json.load(f)
        # trust the feed if captured within the last 5h session window
        if time.time() - d.get("ts", 0) < 5 * 3600 and d.get("session") is not None:
            now = time.time()
...
def claude_usage():
    ...
    # serve cache: fresh (<60s), or in 429 cooldown
    if cached:
...
def claude_usage_fetch():
    ...
    try:
        full = kc_read()
        if not full or "claudeAiOauth" not in full:
            r["err"] = "no creds"
            return r
        oauth = full["claudeAiOauth"]
        ...
        def pick(obj):
            ...
            return (float(u) if u is not None else None), obj.get("resets_at")
